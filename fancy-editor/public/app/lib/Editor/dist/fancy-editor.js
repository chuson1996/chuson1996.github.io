...
function Editor(rootContainer, options) {
    ...
    function cursorEvents() {
        ...
        _helperEventListenerJs2['default'].addEventListener().on(elements.editor)['with']({
            'mouseup keyup blur': function mouseupKeyupBlur() {
                ...
                if (rangy.getSelection().anchorNode && /^(P|H1|H2|PRE|BLOCKQUOTE)$/.test(rangy.getSelection().anchorNode.nodeName.toUpperCase()) && rangy.getSelection().focusNode && /^(P|H1|H2|PRE|BLOCKQUOTE)$/.test(rangy.getSelection().focusNode.nodeName.toUpperCase()) && !rangy.getSelection().anchorNode.textContent.trim() && rangy.getSelection().rangeCount == 1) {
                    showEditorTooltip();
    ...
    function editorMenuEvents() {
        ...
        _helperEventListenerJs2['default'].addEventListener().on(elements.editor)['with']({
            'paste': function paste(e) {
                ...
                var clipboardData = document.createElement('div');
                var block = $(curRange.endContainer).closest("P, BLOCKQUOTE, H1, H2, H3, PRE")[0];
                if (/Firefox/.test(_helperHelper2['default'].getBrowser())) {
                    $(clipboardData).append($('<p>' + (e.originalEvent || e).clipboardData.getData('text/plain') + '</p>'));
                } else {
                    $(clipboardData).append($((e.originalEvent || e).clipboardData.getData('text/html')));
                }

                if (!clipboardData.innerHTML.trim()) return;
                _helperHelper2['default'].node(clipboardData).removeAttributes();
                _helperHelper2['default'].node(clipboardData).editorFormat();
                ...
                if (_helperHelper2['default'].isAtEndOfLine(block, curRange)) {
                    ...
                    //console.log(clipboardData.firstElementChild.cloneNode(false));
                    if (clipboardData.firstChild.nodeName.toUpperCase() == "P" || clipboardData.firstChild.nodeName == block.nodeName) {
                        /* Merge the first node to block */
                        if (block.children.length == 1 && block.firstElementChild.nodeName.toUpperCase() == "BR") {
                            block.firstElementChild.remove();
                        }
                        $(block).append(clipboardData.firstChild.childNodes);
                        block.normalize();
                        clipboardData.firstChild.remove();

                        _helperHelper2['default'].moveCursorTo({ startContainer: block, collapse: false });
                    }
                    if (clipboardData.childNodes.length) {
                        var lastChild = clipboardData.lastElementChild;
                        $(block).after(clipboardData.childNodes);
                        clipboardData.remove();
                    ...
                    if (clipboardData.lastChild.nodeName.toUpperCase() == "P" || clipboardData.lastChild.nodeName == block.nodeName) {
                        // Merge the last node to block
                        _helperHelper2['default'].moveCursorTo({ startContainer: block, collapse: true });
                        console.log(block);
                        $(block).prepend(clipboardData.lastChild.childNodes);
                        block.normalize();
                        clipboardData.lastChild.remove();
                    }
                    if (clipboardData.childNodes.length) {
                        $(block).before(clipboardData.childNodes);
                        clipboardData.remove();
                    }
                } else {
                    (function () {
                        ...
                        if (clipboardData.firstChild.nodeName.toUpperCase() == "P" || clipboardData.firstChild.nodeName == blockL.nodeName) {
                            // console.log('Merge to blockL if possible');
                            $(blockL).append(clipboardData.firstChild.childNodes);
                            blockL.normalize();
                            clipboardData.firstChild.remove();
                        ...
                        /* If clipboardData is one piece, then merge blockR to blockL */
                        if (!clipboardData.children.length) {
                            ...
                            blockL.normalize();
                            blockR.remove();
                        } else if (clipboardData.lastChild.nodeName.toUpperCase() == "P" || clipboardData.lastChild.nodeName == blockR.nodeName) {
                            ...
                            $(blockR).prepend(clipboardData.lastChild.childNodes);
                            blockR.normalize();
                            clipboardData.lastChild.remove();

                            if (clipboardData.childNodes.length) {
                                //console.log('There\'re still some stuff left. After() them in!');
                                $(blockL).after(clipboardData.childNodes);
                                clipboardData.remove();
                            }
                        }

                        if (clipboardData.children.length) {
                            _.forEach(clipboardData.children, function (clipboardChild) {
                                $(blockL).after(clipboardChild);
                                _helperHelper2['default'].moveCursorTo({ startContainer: clipboardChild, collapse: false });
                            });
                        }
                    })();
                }
    ...
    function showEditorTooltip() {
        var selectionRange = rangy.getSelection().getRangeAt(0);
        var pContainer = $(selectionRange.commonAncestorContainer).closest("P, BLOCKQUOTE, H1, H2, H3, PRE")[0];
...
},{"./EditorUndoManager.js":4,"./Figure/Figure":5,"./editor.template.js":6,"./editorDefaultButtons.js":7,"./formatTextM":8,"./helper/EventListener.js":9,"./helper/helper":11,"./linkingM":12}],4:[function(require,module,exports){
/**
...
},{"./Figure/Figure.js":5,"./helper/EventListener.js":9,"./helper/helper":11,"cson-serializer":1,"undo-manager":2}],5:[function(require,module,exports){
/**
...
},{"../helper/EventListener.js":9,"../helper/helper.js":11}],6:[function(require,module,exports){
/**
...
},{"./formatTextM":8,"./helper/EventListener.js":9,"./helper/helper":11,"./linkingM":12}],8:[function(require,module,exports){
/**
...
},{"./helper/helper":11}],9:[function(require,module,exports){
'use strict';
...
exports['default'] = (function () {
    ...
    function moveCursorTo(_ref) {
        ...
        if (collapse !== undefined) {
            var lastTN = _.last(_domJs2['default'](startContainer).findTextNodes());
...
},{"../Figure/Figure.js":5,"./dom.js":10}],12:[function(require,module,exports){
/**
 * Created by chuso_000 on 19/9/2015.
 */
